...
void Background::update() {
	if (vx != 0) {
		...
		//se salió por la parte superior
		if (y - height / 2 > HEIGHT) {
			//Vuelve a aparecer por debajoA
			y = 0 - height / 2;
		}
...
void Background::draw(float scrollX,float scrollY) {//AMPLIACIÓN - ScrollY
	Actor::draw(); // llamar al metodo del hijo

	if (backgroundAux != NULL) {
		// zona sin cubrir por debajo //AMPLIACIÓN -SCROLLY
		if (y + height / 2 > HEIGHT) {
			// pintar aux por debajo
